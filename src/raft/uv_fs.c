...
int UvFsSyncDir(const char *dir, char *errmsg)
{
	 ...
	 *
	 * On POSIX this fsync()s the directory so a segment/snapshot
	 * create/rename is itself durable. The NTFS equivalent is
	 * FlushFileBuffers() on a directory handle opened with
	 * FILE_FLAG_BACKUP_SEMANTICS (required to open a directory at all;
	 * libuv's uv_fs_open() cannot) AND write access: with GENERIC_READ only
	 * it fails with ERROR_ACCESS_DENIED (verified on Windows 11/NTFS). So
	 * open with GENERIC_WRITE and propagate flush failures. If write access
	 * is denied (restrictive ACL), fall back to a read-only open (preserving
	 * POSIX error semantics for missing paths), skip the flush, and rely on
	 * NTFS metadata journaling, which preserves create/rename ORDERING
	 * across a crash -- the guarantee the rename-based commit protocol
	 * needs; the worst case is losing the newest metadata updates. */
...
static int probeDirectIO(int fd, size_t *size, char *errmsg)
{
	...
	*size = 0;
	return 0;
#elif defined(_WIN32)
	/* Windows has no O_DIRECT, and FILE_FLAG_NO_BUFFERING (the Win32
	 * equivalent) is deliberately NOT used. dqlite's Windows write path is
	 * instead: segment files opened write-through (O_DSYNC maps to
	 * FILE_FLAG_WRITE_THROUGH -- see compat/win/dqlite_win_prelude.h) and
	 * written on the libuv threadpool with an explicit fdatasync
	 * (FlushFileBuffers) after every write (uvWriterWorkCbPortable in
	 * uv_writer.c; README.md "Durability on Windows"). Report direct I/O
	 * as unavailable (block size 0) so UvFsProbeCapabilities sets
	 * async = false and the caller selects the threadpool backend,
	 * mirroring the __APPLE__ path above. The explicit early return is
	 * deliberate: the Linux probe below would compile via the compat shims,
	 * but the compat fcntl() fails with ENOSYS, so UvOsSetDirectIo() would
	 * report UV_ENOTSUP -- which the `rv != UV_EINVAL` check would escalate
	 * to RAFT_IOERR rather than "direct I/O unavailable". */
	(void)fd;
	(void)errmsg;
	*size = 0;
	return 0;
