...
#ifdef _WIN32
	/* On Windows munit can't fork, so SQLite stays initialized across
	 * tests; shut it down so the per-test sqlite3_config() calls below
	 * are accepted (they require an uninitialized library). */
	sqlite3_shutdown();
#endif
